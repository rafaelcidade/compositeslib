...
class ply:
	...
	def __init__(self, e1, e2, g12, nu12, thickness, xt=None, xc=None, yt=None, yc=None, s=None,
                     e1t=None, e1c=None, e2t=None, e2c=None, e12s=None, woven=None):
		self.e1 = e1
		self.e2 = e2
		self.g12 = g12
		self.nu12 =nu12
	...
	# [matrix<0> = bottom face value , matrix<1> = top face value]
		self.local_stress = np.matrix([[0. for col in range(2)] for row in range(3)])
		self.local_strain = np.matrix([[0. for col in range(2)] for row in range(3)])
		self.global_stress = np.matrix([[0. for col in range(2)] for row in range(3)])
		self.global_strain = np.matrix([[0. for col in range(2)] for row in range(3)])
	...
	def q(self):
		...
		return np.matrix([   [ e1 / (1 - nu12*nu21), nu12*e2 / (1 - nu12*nu21), 0. ],
...
class laminate():
	def __init__(self, input_plies):
		...
		self.S = self.C.I; 
		self.rho = (2 * self.S[0,1] + self.S[2,2]) / (2 * math.sqrt(self.S[0,0]*self.S[1,1]))
...
class load(object):
	def __init__(self, nx=None, ny=None, nxy=None, mx=None, my=None, mxy=None):
		self.nx  = (0 if nx == None else nx)
		self.ny  = (0 if ny == None else ny)
		self.nxy = (0 if nxy == None else nxy)
		self.mx  = (0 if mx == None else mx)
		self.my  = (0 if my == None else my)
		self.mxy = (0 if mxy == None else mxy)