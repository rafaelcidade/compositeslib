...
def plot_stress(laminate):
    ...
    for ply in laminate.plies:
        ...
        for i in range(3):
            stress_array[i].extend(
                [round(ply.global_stress[i, 0], 2), round(ply.global_stress[i, 1], 2)])
            stress_array[i + 3].extend([round(ply.local_stress[i, 0], 2),
                                       round(ply.local_stress[i, 1], 2)])
...
def plot_strain(laminate):
    ...
    for ply in laminate.plies:
        ...
        z_array.extend([ply.z[0], ply.z[1]])
        for i in range(3):
            strain_array[i].extend(
                [ply.global_strain[i, 0] * 100, ply.global_strain[i, 1] * 100])
            strain_array[i +
                         3].extend([ply.local_strain[i, 0] *
                                    100, ply.local_strain[i, 1] *
                                    100])
...
def plot_max_stress(ply):
    ...
    plt.plot([-ply.xc + -ply.xc / 2, ply.xt +
             ply.xt / 2], [0, 0], color='black')
    plt.plot([0, 0], [-ply.yc + -ply.yc / 2,
             ply.yt + ply.yt / 2], color='black')
    # Set axis limits
