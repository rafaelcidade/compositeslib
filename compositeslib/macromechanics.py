...
def calculate_stress(laminate, load):
    load_array = np.matrix(
        [load.nx, load.ny, load.nxy, load.mx, load.my, load.mxy])
    laminate.strain_curvature = laminate.ABD().I * load_array.T

    for ply in laminate.plies:
        ply.global_strain[:, 0] = laminate.strain_curvature[:3] + \
            ply.z[0] * laminate.strain_curvature[3:]
        ply.global_strain[:, 1] = laminate.strain_curvature[:3] + \
            ply.z[1] * laminate.strain_curvature[3:]
...
def max_stress(ply):
    if not validate_strength(ply):
        return
    long_index = max(ply.local_stress[0, 0] /
                     ply.xt if ply.local_stress[0, 0] > 0 else -
                     ply.local_stress[0, 0] /
                     ply.xc, ply.local_stress[0, 1] /
                     ply.xt if ply.local_stress[0, 1] > 0 else -
                     ply.local_stress[0, 1] /
                     ply.xc)

    trans_index = max(ply.local_stress[1, 0] /
                      ply.yt if ply.local_stress[1, 0] > 0 else -
                      ply.local_stress[1, 0] /
                      ply.yc, ply.local_stress[1, 1] /
                      ply.yt if ply.local_stress[1, 1] > 0 else -
                      ply.local_stress[1, 1] /
                      ply.yc)

    shear_index = max(ply.local_stress[2, 0] /
                      ply.s if ply.local_stress[2, 0] > 0 else -
                      ply.local_stress[2, 0] /
                      ply.s, ply.local_stress[2, 1] /
                      ply.s if ply.local_stress[2, 1] > 0 else -
                      ply.local_stress[2, 1] /
                      ply.s)

    bot_index = max(ply.local_stress[0, 0] /
                    ply.xt if ply.local_stress[0, 0] > 0 else -
                    ply.local_stress[0, 0] /
                    ply.xc, ply.local_stress[1, 0] /
                    ply.yt if ply.local_stress[1, 0] > 0 else -
                    ply.local_stress[1, 0] /
                    ply.yc, abs(ply.local_stress[2, 0]) /
                    ply.s)

    top_index = max(ply.local_stress[0, 1] /
                    ply.xt if ply.local_stress[0, 0] > 0 else -
                    ply.local_stress[0, 1] /
                    ply.xc, ply.local_stress[1, 1] /
                    ply.yt if ply.local_stress[1, 0] > 0 else -
                    ply.local_stress[1, 1] /
                    ply.yc, abs(ply.local_stress[2, 1]) /
                    ply.s)
...
def max_strain(ply):
    if not validate_strength(ply):
        return
    long_index = max(ply.e1 *
                     ply.local_strain[0, 0] /
                     ply.xt if ply.e1 *
                     ply.local_strain[0, 0] > 0 else -
                     ply.e1 *
                     ply.local_strain[0, 0] /
                     ply.xc, ply.e1 *
                     ply.local_strain[0, 1] /
                     ply.xt if ply.e1 *
                     ply.local_strain[0, 1] > 0 else -
                     ply.e1 *
                     ply.local_strain[0, 1] /
                     ply.xc)

    trans_index = max(ply.e2 *
                      ply.local_strain[1, 0] /
                      ply.yt if ply.e2 *
                      ply.local_strain[1, 0] > 0 else -
                      ply.e2 *
                      ply.local_strain[1, 0] /
                      ply.yc, ply.e2 *
                      ply.local_strain[1, 1] /
                      ply.yt if ply.e2 *
                      ply.local_strain[1, 1] > 0 else -
                      ply.e2 *
                      ply.local_strain[1, 1] /
                      ply.yc)
...
def tsai_wu(ply):
    ...
    return max(F11 *
               ply.local_stress[0, 0]**2 +
               F12 *
               ply.local_stress[0, 0] *
               ply.local_stress[1, 0] +
               F22 *
               ply.local_stress[1, 0]**2 +
               F66 *
               ply.local_stress[2, 0]**2 +
               F1 *
               ply.local_stress[0, 0] +
               F2 *
               ply.local_stress[1, 0], F11 *
               ply.local_stress[0, 1]**2 +
               F12 *
               ply.local_stress[0, 1] *
               ply.local_stress[1, 1] +
               F22 *
               ply.local_stress[1, 1]**2 +
               F66 *
               ply.local_stress[2, 1]**2 +
               F1 *
               ply.local_stress[0, 1] +
               F2 *
               ply.local_stress[1, 1])
...
def hashin(ply):
    if not validate_strength(ply):
        return
    fiber_index = max(ply.local_stress[0, 0] /
                      ply.xt if ply.local_stress[0, 0] > 0 else -
                      ply.local_stress[0, 0] /
                      ply.xc, ply.local_stress[0, 1] /
                      ply.xt if ply.local_stress[0, 1] > 0 else -
...
def linde(ply):
    ...
    fiber_index = max((1 /
                       ply.e1t) *
                      math.sqrt(abs((ply.e1t /
                                     ply.e1c) *
                                    ply.local_strain[0, 0]**2 +
                                    (ply.e1t -
                                     ply.e1t**2 /
                                     ply.e1c) *
                                    ply.local_strain[0, 0])), (1 /
                                                               ply.e1t) *
                      math.sqrt(abs((ply.e1t /
                                     ply.e1c) *
                                    ply.local_strain[0, 1]**2 +
                                    (ply.e1t -
                                     ply.e1t**2 /
                                     ply.e1c) *
                                    ply.local_strain[0, 1])))
    matrix_index = max((1 /
                        ply.e2t) *
                       math.sqrt(abs((ply.e2t /
                                      ply.e2c) *
                                     ply.local_strain[1, 0]**2 +
                                     (ply.e2t -
                                      ply.e2t**2 /
                                      ply.e2c) *
                                     ply.local_strain[1, 0] +
                                     ((ply.e2t /
                                       ply.e12s)**2) *
                                     ply.local_strain[2, 0]**2)), (1 /
                                                                   ply.e2t) *
                       math.sqrt(abs((ply.e2t /
                                      ply.e2c) *
                                     ply.local_strain[1, 1]**2 +
                                     (ply.e2t -
                                      ply.e2t**2 /
                                      ply.e2c) *
                                     ply.local_strain[1, 1] +
